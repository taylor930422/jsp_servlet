...
<body>
	...
	<div>
		<button class="btn btn-primary float-end me-2 opacity-75" onclick="location.href='${boardUrl }/add'">글 작성</button>
	</div>
	</c:if>
	<div>
		<form action="${boardUrl }/delete" method="post">
		<button class="btn btn-danger float-start ms-2 opacity-75" type="submit">삭제</button>
		<table class="table">
			<thead>
				<tr>
					<th></th>
			...
			<tbody>
				<c:forEach var="data" items="${requestScope.paging.data}">
					<tr>
						...
						<td><a class="a" href="${boardUrl }/detail?id=${data.id}">${data.title}</a></td>
						<td>${data.writer }</td>
						<td>${data.createDate }</td>
						<td>${data.viewCnt }</td>
						<td><a class="a text-danger opacity-75" href="${boardUrl }/delete?id=${data.id }">삭제</a>
					</tr>
				</c:forEach>
			</tbody>
		</table>
